...
public class KPABEAccessTree implements Iterable<KPABEAccessTree.Node> {
    ...
    public KPABEAccessTree(Node root) {
        this.root = root;
    }
    ...
    private Element decryptNodeHelper(Node n, int[] messageAttributes, Map<Integer, Element> Di, Map<Integer, Element> Ei, Pairing bp) {
        ...
        for (int i = 0; i < n.children.size(); i++){
            Node childNode = n.children.get(i);
            // 递归调用，恢复子节点的秘密值
            Element childSecret = decryptNodeHelper(childNode, messageAttributes, Di, Ei, bp);
            if (childSecret != null){
    ...
    public Element decryptNode(int[] messageAttributes, Map<Integer, Element> Di, Map<Integer, Element> Ei, Pairing bp) {
